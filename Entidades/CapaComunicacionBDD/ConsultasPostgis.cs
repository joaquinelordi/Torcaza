...
namespace Entidades.CapaComunicacionBDD
{
    ...
    public static class PostgisTransforms
    {
        ...
        public static async Task<List<TuplaCalculo>> TransformarTuplasGpsA22185Async(
            NpgsqlConnection conn,
            IReadOnlyList<TuplaCalculo> tuplasGps,
            CancellationToken ct = default)
        {
            if (tuplasGps is null || tuplasGps.Count == 0)
                return new List<TuplaCalculo>();
            try
            {
                ...
                const string sql = @"
                    WITH data AS (
                      SELECT
                        ...
                        (j->>'R')::double precision   AS r,
                        (j->>'Var')::double precision AS var
                    ...
                    SELECT ST_X(g) AS x, ST_Y(g) AS y, r, var
                    FROM anclas;";

                using var cmd = new NpgsqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("p", NpgsqlDbType.Jsonb, json);

                var lista = new List<TuplaCalculo>(tuplasGps.Count);
                ...
                */
                while (await rd.ReadAsync(ct))
                {
                    ...
                    double r = rd.IsDBNull(2) ? double.NaN : rd.GetFieldValue<double>(2);
                    double v = rd.IsDBNull(3) ? double.NaN : rd.GetFieldValue<double>(3);
                    lista.Add(new TuplaCalculo(x, y, r, v));
                }
                return lista;
            }
            catch (Exception ex)
        ...
        public static async Task<Vector2> Transformar22185AWgsAsync(
            NpgsqlConnection conn,
            Vector2 xy22185,
            CancellationToken ct = default)
        {
            ...
            using var cmd = new NpgsqlCommand(sql, conn);
            cmd.Parameters.AddWithValue("x", NpgsqlDbType.Double, (double)xy22185.X);
            cmd.Parameters.AddWithValue("y", NpgsqlDbType.Double, (double)xy22185.Y);

            await using var rd = await cmd.ExecuteReaderAsync(ct);
            if (await rd.ReadAsync(ct))
            {
                var lon = rd.GetDouble(0);
